...
class EpsilonGreedy:
    ...
    def act(self) -> int:
        ...
        if np.random.random() < self.epsilon:
            ...
            exploratory_action  = np.random.randint(low = 0, high = self.num_actions)  # TODO: Implement this assignment
    ...
    def simple_update(self, action: int, reward: float) -> None:
        ...
        self.q_values[action] = self.q_values[action] + (1/self.action_counts[action])*(reward - self.q_values[action])
